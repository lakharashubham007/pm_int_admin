...
const ListTax = () => {
    ...
    const fetchTaxes = async () => {
        setIsLoading(true);
        try {
            const data = await productService.getTaxes({ masterCategory });
            if (data.success) {
                setTaxes(data.taxes);
            } else {
            ...
            toast.error(error.message || 'Error loading taxes');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const currentRows = filteredTaxes.slice(indexOfFirstRow, indexOfLastRow);
        ...
        <div className="category-page-container fade-in">
            <div className="category-content-pane">
                <header className="internal-page-header" style={{ padding: '0 0 1.5rem 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ flex: 1 }}>
                        <h1 style={{ fontSize: '1.5rem', marginBottom: '4px' }}>Tax Management</h1>
                        <p style={{ margin: 0, color: 'hsl(var(--muted-foreground))', fontSize: '0.9rem' }}>Configure and manage tax brackets for financial accuracy.</p>
                    </div>
                    ...
                    <div className="header-actions" style={{ flex: 1, justifyContent: 'flex-end' }}>
                        <button className="secondary-button" onClick={fetchTaxes} title="Refresh Data" style={{ padding: '0.5rem 1rem' }}>
                ...
                <div className="category-table-wrapper">
                        ...
                        <tbody>
                            {isLoading ? (
                                    ...
                                    <td colSpan="3" style={{ textAlign: 'center', padding: '4rem 0' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                                            <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'hsl(var(--secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Landmark size={32} style={{ color: 'hsl(var(--muted-foreground))' }} />
                                            </div>
                                            <div>
                                                <h3 style={{ margin: '0 0 0.5rem 0', color: 'hsl(var(--foreground))', fontSize: '1.2rem' }}>No Taxes Found</h3>
                                                <p style={{ margin: 0, color: 'hsl(var(--muted-foreground))' }}>Establish financial compliance by adding your first tax.</p>
                                            </div>
                                ...
                                currentRows.map((tax, index) => (
                                    <tr key={tax._id} className="category-row">
                                        <td style={{ textAlign: 'center', color: 'hsl(var(--muted-foreground))', fontWeight: '500' }}>
                                            {indexOfFirstRow + index + 1}
                                        </td>
                                        <td>
                                            <div style={{ fontWeight: '600', fontSize: '0.95rem' }}>{tax.name}</div>
                                            <div style={{ fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))' }}>ID: {tax._id.slice(-6).toUpperCase()}</div>
                                        </td>
                                        <td>
                    ...
                    {!isLoading && filteredTaxes.length > 0 && (
                        <div className="pagination-footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', borderTop: '1px solid hsl(var(--border) / 0.3)' }}>
                            <div style={{ fontSize: '0.85rem', color: 'hsl(var(--muted-foreground))' }}>
                                <span>Showing {indexOfFirstRow + 1} to {Math.min(indexOfLastRow, filteredTaxes.length)} of {filteredTaxes.length} entries</span>
                            </div>
