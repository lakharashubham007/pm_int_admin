...
const EditProduct = () => {
        ...
        const loadAllData = async () => {
            ...
            try {
                const [cat, sub, br, un, tx, attr, productRes] = await Promise.all([
                    ...
                    vendorProductService.getVendorProductById(id)
                ]);
                ...
                setMasterCategory(p.masterCategory || 'Grocery');

                setMasters({
                    categories: cat.categories || [],
                    allSubCategories: allSubCats,
                    subCategories: p.categoryId ? allSubCats.filter(s => s.categoryId?._id === (p.categoryId?._id || p.categoryId)) : [],
                    brands: br.brands || [],
                    units: un.units || [],
                    taxes: tx.taxes || [],
                ...
                setFormData({
                    ...
                    variants: p.variants ? p.variants.map(v => ({
                        ...
                        variantValues: v.attributes.map(a => ({
                            ...
                            valueName: a.valueId?.name || a.valueId?.valueName || '',
                        ...
                        mrp: v.pricing?.mrp || '',
                        discount: v.pricing?.discountValue || 0,
                ...
                if (p.productType === 'Variant' && p.variants?.length > 0) {
                    ...
                    await Promise.all(Array.from(attrIdsToFetch).map(async (typeId) => {
                        ...
                        if (masterAttr) {
                            try {
                                const res = await productService.getVariantValues({ variantTypeId: typeId });
                                const values = res.variantValues || [];
                                values.sort(unitAwareSort);
                                masterAttr.values = values;
                            } catch (e) {
                ...
                console.error(error);
                toast.error('Failed to load vendor product data');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const updateVariantRow = (index, field, value) => {
        ...
        if (field === 'mrp' || field === 'sellingPrice' || field === 'price') {
            const mrp = parseFloat(updated[index].mrp) || 0;
            const sp = parseFloat(updated[index].sellingPrice || updated[index].price) || 0;
            if (mrp > 0 && sp > 0 && mrp > sp) {
        ...
        setFormData(prev => ({
            ...
            variants: prev.variants.map(v => {
                const newVariant = {
                    ...
                    ...(bulkApply.price !== '' && { price: bulkApply.price, sellingPrice: bulkApply.price }),
    ...
    const addCustomVariantRow = () => {
        setFormData(prev => ({
            ...prev,
            variants: [{
                sku: '', mrp: '', price: '', sellingPrice: '', discount: '', costPrice: '', taxId: '', unitId: '', quantity: '', minStock: '',
                variantValues: [] // Allow manual entry without matrix
            }, ...prev.variants]
        }));
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!cleanPricing.taxId) cleanPricing.taxId = null;
            ...
            if (formData.productType === 'Variant') {
                const cleanVariants = formData.variants.map(v => {
                    const cleanV = { ...v };
                    if (!cleanV.taxId) cleanV.taxId = null;
                    if (!cleanV.unitId) cleanV.unitId = null;
                    return cleanV;
                });
            ...
            await vendorProductService.updateVendorProduct(id, data);
            toast.success('Vendor product updated successfully!');
            navigate('/vendors/products/list');
        } catch (error) {
            toast.error(error.response?.data?.message || 'Failed to update vendor product');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="product-page-container fade-in">
            <div className="product-content-pane">
                <header className="internal-page-header" style={{ padding: '1.5rem 0', display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'stretch' }}>
                    ...
                    <div className="product-tab-navbar" style={{ width: '100%', marginTop: '0.5rem', background: 'hsl(var(--background) / 0.8)' }}>
                        {[
                            ...
                            <div key={tab.id} className={`tab-nav-item ${activeTab === tab.id ? 'active' : ''}`} onClick={() => setActiveTab(tab.id)} style={{ flex: 1, justifyContent: 'center', border: '1px solid hsl(var(--border)/0.2)' }}>
                                {tab.icon}{tab.label}
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="animate-in fade-in">
                            <div className="product-grid">
                                ...
                                <div className="product-field-group">
                                    ...
                                    <CustomSelect
                                        options={masters.subCategories.map(s => ({ value: s._id, label: s.name }))}
                                        value={formData.subCategoryId}
                                        onChange={(val) => setFormData(prev => ({ ...prev, subCategoryId: val }))}
                                        placeholder={formData.categoryId ? 'Awaiting Category...' : 'Select Sub Category'}
                                ...
                                <div className="product-field-group">
                                    ...
                                    <CustomSelect options={masters.brands.map(b => ({ value: b._id, label: b.name }))} value={formData.brandId} onChange={(val) => setFormData(prev => ({ ...prev, brandId: val }))} placeholder="Select Brand" />
                                </div>
                                <div className="product-field-group">
                                    <label className="product-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <span>Base Unit</span>
                                    ...
                                    <CustomSelect options={masters.units.map(u => ({ value: u._id, label: u.name }))} value={formData.unitId} onChange={(val) => setFormData(prev => ({ ...prev, unitId: val }))} placeholder="Select Unit" />
                                </div>
                                <div className="product-field-group" style={{ gridColumn: '1 / -1' }}>
                                        ...
                                        <div className="vb-step-card" style={{ padding: 0, overflow: 'hidden' }}>
                                            ...
                                            <div className="vb-compact-toolbar">
                                                ...
                                                <div className="vb-bulk-bar" style={{ padding: '0.5rem 1rem' }}>
                                                    ...
                                                    <button
                                                        onClick={(e) => { e.preventDefault(); setShowBulkModal(true); }}
                                                    ...
                                                    <div style={{ paddingBottom: '1rem' }}>
                                                        {formData.variants.map((variant, idx) => (
                                                            <div key={idx} className="vb-variant-row">
                                                                ...
                                                                {/* Stock */}
                                                                <div>
                                                                    <input type="number" className="product-input vb-row-input" value={variant.quantity} onChange={e => updateVariantRow(idx, 'quantity', e.target.value)} placeholder="0" />
                                                                </div>
                                                                ...
                                                                {/* Delete */}
                                                                <div style={{ borderRight: 'none', justifyContent: 'center' }}>
                ...
                <div className="action-footer" style={{ borderBottomLeftRadius: '20px', borderBottomRightRadius: '20px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                        {activeTab > 1 && (
                            <button className="btn-premium-outline" onClick={() => setActiveTab(activeTab - 1)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    onSuccess={(newSubcategory) => {
                        ...
                        if (subCatParentId === formData.categoryId) {
                            ...
                            setMasters(prev => {
                                ...
                                return { ...prev, subCategories: filtered };
                            });
                        }
                    }}
                />
            ...
            {showValueModal && activeAttributeForValue && (
                <QuickCreateModal
                    ...
                    type="Value"
                    masters={{ variantTypeId: activeAttributeForValue._id, inputType: activeAttributeForValue.inputType }}
                    onSuccess={(newValue) => {
                    ...
                    <div className="quick-modal-container" style={{ maxWidth: '600px' }}>
                        ...
                        <div className="quick-modal-content">
                            <div className="quick-modal-header" style={{ marginBottom: '1.5rem', alignItems: 'flex-start' }}>
                                ...
                                <button className="quick-modal-close-btn" onClick={(e) => { e.preventDefault(); setShowBulkModal(false); }} title="Close Modal">
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <form className="quick-modal-form" style={{ gap: '1rem' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    <div className="quick-modal-form-group">
                                        ...
                                        <input type="number" className="quick-modal-input" value={bulkApply.price} onChange={e => setBulkApply(p => ({ ...p, price: e.target.value }))} placeholder="0.00" />
                                    </div>
                                    <div className="quick-modal-form-group">
                                ...
                                <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                                    <button
                                        ...
                                        onClick={(e) => { e.preventDefault(); setShowBulkModal(false); }}
                                    >
                                        Cancel
                                    </button>
                                    <button
