...
import {
    ArrowLeft, Save, Building2, Landmark, User, Zap, Box, Layers, Image as ImageIcon, FileText, ChevronRight, ChevronLeft, MapPin, X, RefreshCw, Plus, Trash2, UploadCloud, File
} from 'lucide-react';
...
const EditVendor = () => {
    ...
    useEffect(() => {
        if (paramId) {
            const fetchVendor = async () => {
                try {
                    const response = await vendorService.getVendorById(paramId);
    ...
    const handleSaveAndProceed = async (targetTab) => {
        try {
            ...
            if (response.success) {
                if (!vendorId && response.data && response.data._id) {
                    setVendorId(response.data._id);
                    window.history.pushState(null, '', `/vendors/edit-vendor/${response.data._id}`);
                }

                if (targetTab === 'submit') {
                    toast.success(isEdit ? 'Vendor profile synchronized' : 'Vendor created successfully');
                    navigate('/vendors/list-vendors');
                } else {
                ...
                <header className="employee-header">
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                            ...
                            <h1>{isEdit ? 'Revise Vendor Architecture' : 'Establish Vendor Entity'}</h1>
                        </div>
                        <p>{isEdit ? `Modifying intelligence for ${formData.businessName}` : 'Configure a new supply chain entity across 5 stages.'}</p>
                    </div>
                        ...
                        {activeTab === 1 && (
                            <div className="employee-form-grid" style={{ gap: '1.5rem' }}>
                                ...
                                <div className="employee-form-group">
                                    ...
                                    <input name="password" type="password" className="enterprise-input" value={formData.password} onChange={handleInputChange} placeholder="••••••••" />
                                </div>
                                <div className="employee-form-group">
                                    <label>Confirm Password</label>
                                    <input name="confirmPassword" type="password" className="enterprise-input" value={formData.confirmPassword} onChange={handleInputChange} placeholder="••••••••" />
                                </div>
