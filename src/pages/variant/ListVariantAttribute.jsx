...
import { Plus, Search, Edit3, Trash2, Filter, RefreshCw, ChevronLeft, ChevronRight, ChevronDown, Settings2, Eye, ShoppingCart, UtensilsCrossed } from 'lucide-react';
...
const ListVariantAttribute = () => {
    ...
    const fetchAttributes = async () => {
        setIsLoading(true);
        try {
            const data = await productService.getVariantAttributes({ masterCategory });
            if (data.success) {
        ...
        <div className="category-page-container fade-in">
            <div className="category-content-pane">
                <header className="internal-page-header" style={{ padding: '0 0 1.5rem 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ flex: 1 }}>
                        <h1 style={{ fontSize: '1.5rem', marginBottom: '4px' }}>Variant Attributes</h1>
                        <p style={{ margin: 0, color: 'hsl(var(--muted-foreground))', fontSize: '0.9rem' }}>Define master attributes like Size, Color, and Weight for your products.</p>
                    </div>
                    ...
                    <div className="header-actions" style={{ flex: 1, justifyContent: 'flex-end' }}>
                        <button className="secondary-button" onClick={fetchAttributes} title="Refresh Data" style={{ padding: '0.5rem 1rem' }}>
