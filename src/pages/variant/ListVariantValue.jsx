...
import { Plus, Search, Edit3, Trash2, Filter, RefreshCw, ChevronLeft, ChevronRight, ChevronDown, ListTree, ArrowLeft, ShoppingCart, UtensilsCrossed } from 'lucide-react';
...
const ListVariantValue = () => {
    ...
    useEffect(() => {
        fetchValues();
    }, [attributeId, masterCategory]);

    const fetchValues = async () => {
        setIsLoading(true);
        try {
            const data = await productService.getVariantValues({ variantTypeId: attributeId, masterCategory });
            if (data.success) {
            ...
            <div className="category-content-pane">
                <header className="internal-page-header" style={{ padding: '0 0 1.5rem 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ flex: 1 }}>
                        ...
                        <h1 style={{ fontSize: '1.5rem', marginBottom: '4px' }}>{attribute.name} Values</h1>
                        <p style={{ margin: 0, color: 'hsl(var(--muted-foreground))', fontSize: '0.9rem' }}>Manage specific variations for the "{attribute.name}" attribute.</p>
                    </div>
                    ...
                    <div className="header-actions" style={{ flex: 1, justifyContent: 'flex-end' }}>
                        <button className="secondary-button" onClick={fetchValues} title="Refresh Data" style={{ padding: '0.5rem 1rem' }}>
